...
public class GitStatus extends AbstractModelObject implements UnsecuredRootAction{
    ...
    public HttpResponse doNotifyCommit(@QueryParameter(required = true) String url, @QueryParameter(required = false) String branches) throws ServletException, IOException {
        ...
        for (Listener listener : Hudson.getInstance().getExtensionList(Listener.class)) {
    ...
    /**
     * Used to test if what we have in the job configuration matches what was
     * submitted to the notification endpoint. It is better to match loosely and
     * wastes a few polling calls than to be pedantic and miss the push
     * notification, especially given that Git tends to support multiple access
     * protocols.
     */
    ...
    public static abstract class Listener implements ExtensionPoint {
         ...
         * @return any response contributors for the response to the push
         * request.
         * @since 1.4.1
         */
    ...
    /**
     * Handle standard {@link AbstractProject} instances with a standard
     * {@link SCMTrigger}.
     *
     * @since 1.4.1
     */
    ...
    public static class HudsonAbstractProjectListener extends Listener {
        ...
        /**
         * A response contributor for triggering polling of an
